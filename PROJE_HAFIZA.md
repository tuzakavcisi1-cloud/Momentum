...
- **Kalan kurulumlar → Onur'a runbook:** `araclar/KURULUM-kalan-adimlar.md` (Android cmdline-tools + VS C++ Build Tools + Docker/WSL2, kendi terminalinde, tek reboot sonda). Bunlar slice-2a'ya gerekmez. **Ortam bulgusu:** DC/bazı kabuklar SDK'sız x86 `dotnet`'i öne alıyor; 64-bit SDK 9.0.316 kurulu ve Claude Code doğru kullanıyor (PATH hijyeni opsiyonel).
- **Açık:** DB slice-2b'de Docker Compose. Oturum 4 **SARI** → slice-2a temiz oturumda ("başla").
