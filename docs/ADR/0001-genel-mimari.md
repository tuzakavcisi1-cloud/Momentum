...
**Errata (2 Ağustos 2026, oturum 48 — K111, Onur kilitledi): hedef çatı `net9.0` → `net10.0`.** SDK pini `global.json` = **10.0.302**. Karar kâğıtta değil **koşan deneyle** alındı: `verify.ps1` net10.0'da **EXIT 0** (build **0 uyarı / 0 hata** · test **120/120** · CVE **0**). Üç ölçülmüş sonuç:
- **① Geçiş ATOMİKTİR.** SDK'yı tek başına yükseltip `TargetFramework`'ü `net9.0` bırakmak depoyu **kırar** (`CS0023`, `OrSetProperties.cs` 35/86). İzole mutantla kanıtlandı (depoya dokunulmadan): `net9.0`+C#14 **KIRIK** · `net9.0`+`LangVersion=13` **DERLENDİ** · `net10.0`+C#14 **DERLENDİ** ⇒ suçlu SDK değil, **C# 14 span dönüşümleri + net9 referans kümesi**: `dizi.Reverse()` çağrısı `MemoryExtensions.Reverse<T>(Span<T>)` (**void**) aşırı yüklemesine bağlanıyor; `net10.0`'da `Enumerable.Reverse<T>(T[])` bulunduğu için doğru bağlanma geri geliyor. Bu depo `LangVersion=latest` taşıdığı için bu sınıfa **açıktır** ve bu, K-H1'in kendi kapı ailesine ait bir risktir.
- **② *"En güncel sürüme çık"* naif kuralı GEÇERSİZDİR** ve CVE kapısı (red line #3) bunu ölçerek gösterdi: `Microsoft.AspNetCore.OpenApi 10.0.10` transitif olarak **savunmasız** `Microsoft.OpenApi 2.0.0` getirir (GHSA-v5pm-xwqc-g5wc, **High / CVSS 7.5**, uncontrolled recursion; ilk yamalı sürüm **2.7.5**) ⇒ `Momentum.Api`'ye **CVE pini `Microsoft.OpenApi 2.11.0`** eklendi. **Bu bir mimari karar değildir**: yukarı akış güvenli bir transitif sürüme geçtiğinde satır **silinir** (ölçüm: `dotnet nuget why Momentum.sln Microsoft.OpenApi`). Ayrıca `Microsoft.EntityFrameworkCore` **10.0.4**'te tutulur — `Npgsql.EntityFrameworkCore.PostgreSQL 10.0.3` o Relational sürümünü bekler ve 10.0.10 pini `MSB3277` çakışması üretir. **Kural: sağlayıcı zinciriyle hizala, sürüm numarasını değil.**
- **③ Kâğıtta *"en ciddi risk"* denen `Mediator` 3.0.2 HİÇ ISIRMADI** (netstandard2.0 hedefler; source generator'ın .NET sürümüne göre ayrı sürümü olmaz). Ölçülmemiş bir riskin adı, ölçülen riskin yerini tutmadı.

  **Bu erratanın kapsamı:** bu belgedeki *".NET 9"* ifadeleri (§1 kapsam cümlesi ve **K-D1**'in *".NET 9 yerleşik JSON / .NET 9'da yerleşik Swagger UI yok"* notu dâhil) bundan sonra **net10** olarak okunur; yerinde düzeltilmez. **Kanıt:** `KANIT/net10-adim6/` (7 ham verify/build logu + mutant betiği + çıktısı + ölçüm özeti). **Kapsam dışı bırakılan ve BORÇ yazılan paketler:** `Asp.Versioning.Http` 8.1.1 (→10.0.1, iki majör), `xunit` 2.9.2, `Microsoft.NET.Test.Sdk` 17.12.0, `xunit.runner.visualstudio` 2.8.2 (→3.1.5), `Scalar.AspNetCore` 2.16.15. **Beyan edilmiş sınır:** ölçüm **tek makinede** yapıldı; CI'da .NET 10 kurulumu **[DOĞRULANMADI]**.
