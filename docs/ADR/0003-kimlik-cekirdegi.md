...
| **Sızmış/yaygın parola kara listesi** | **VARDIR — gömülü, `[KS-31]`** | **NIST SP 800-63B-4'ün İKİNCİ `SHALL`'ı** (aşağıda birebir). v5 ilk `SHALL`'ı (`[KS-17]`) *"NIST çizgisi"* diye savunurken bunu **ne karara bağlamış ne kapsam dışı ilan etmişti** ⇒ **adlandırılmamış sapma** (K28-c). |
...
**Liste dosyası kırmızı çizgi #3 kapısından geçer [ÖLÇÜLMEDİ — kapı `GOREV-slice-3c-auth` spec'inde koşar]:** kaynağın lisansı **izinli OSI ailesinde** (MIT/Apache-2.0/BSD-3-Clause) olmalıdır ve `dependencies.md`'ye satır olarak girer. **Bu belge belirli bir kaynak PİNLEMEZ** — bugün ölçülmemiş bir lisans iddiası yazmak, projenin `Isopoh` kaleminde adlandırdığı hatanın aynısı olurdu. Kapı düşerse geri dönüş **adlandırılmıştır ve şimdiden onaylıdır:** liste, izinli lisanslı bir kaynaktan **kendi üretilmiş** bir alt kümeye indirilir ya da `[KS-31]` küçültülür; **kaleme tamamen vazgeçilmez.**
**Kapı: M56** (`B`): mutasyon kontrolü kaldırır; kill sinyali *"kara listedeki bir parola (`123456`) `400` alır **ve** listede olmayan, `[KS-17]`'yi sağlayan bir parola `201` alır"* **FAIL**. İkinci ayak zorunludur: tek ayakla *"her şeyi reddet"* mutasyonu da testi geçerdi.
...
11. **[K14-a'nın kabul edilmiş bedeli — v6'da YENİDEN TARTILDI]** Replay-idempotency penceresi (`[KS-4]`) çalınmış bir token için reuse-detection'ı **kaldırmaz, bir ÇARPIŞMAYA kadar erteler** (§2-C(3)'ün dürüst muhasebesi): dal (d)'ye ancak tüketilmiş token ya `[KS-4]` **dışında** ya da **halefi tüketilmişken** sunulduğunda varılır. **Gecikmenin ölçüsü `[KS-4]` DEĞİLDİR:** çevrimiçi kurbanda meşru istemcinin bir sonraki yenilemesine, yani **`[KS-1]`**'e kadar; **kurban çevrimdışıysa ailenin mutlak ömrü `[KS-2]`'ye kadar** uzar. **Ayrıca gölgeleme mümkündür:** koşul 2 **halefin** `consumed_at`'ine baktığı için iki taraf zinciri adım adım paylaşabilir — ömür uzamaz, yeni token basılmaz, ama tespit ertelenmeye devam eder. *(v4/v5 burada `[KS-4]` yazıyor ve *"halef kullanılınca aile düşer"* diyordu; **her iki iddia da §0.4'te geri çekildi**.)* **Sahiplik:** mekanizmanın sahibi **K14-a**, `[KS-4]`'ün değeri **K28-a ile Onur tarafından kilitlenmiştir** — v3/v4'ün **K16-b** atfı **yanlıştı** (K16-b'nin tam metni yalnız hız sınırı tavanlarıdır: `[KS-10]`/`[KS-11]`/`[KS-12]`).
