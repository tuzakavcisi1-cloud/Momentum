...
- 🔴 **`B-O63-1` — GÖLGELEME KAPISI YOK.** İstemci kökünde `v1`/`health`/`hubs`/`scalar`/`openapi` **adında bir dosya** bulunursa `UseStaticFiles` aynı adlı uç noktayı gölgeler; ölçüm o63'te **bir kez** koştu (21 vaka, temiz), **mekanik kapı yok**. Kapanış: `izolasyon-olc.py`'ye *"istemci kökünde API öneki adında dosya var mı"* ayağı + iki mutant (dosya koy ⇒ ısırsın · koyma ⇒ sussun).
- 🔴 **`B-O63-2` — `--no-web-resources-cdn` CI'DA ZORLANMIYOR.** o63'te ölçüldü: Flutter'ın **varsayılan** build'i CanvasKit'i `gstatic`'ten çeker ve `require-corp` CORP'suz çapraz-köken kaynağı **fiilen bloklar** (pozitif+negatif kontrollü). ⇒ Bayrak bir tercih değil **kapı şartıdır**, ama hiçbir kapı onu ölçmüyor. Kapanış: build çıktısının `flutter_bootstrap.js`'inde `useLocalCanvasKit":true` arayan statik ayak + mutant.
- 🟡 **`B-O63-3` — `SpaDisiOnEkler` LİSTESİNİN TAZELİĞİ ÖLÇÜLMÜYOR.** `IstemciServisi.cs`'teki beş önek (`/v1`,`/health`,`/hubs`,`/scalar`,`/openapi`) elle yazıldı; yeni bir kök yol eklenirse (örn. `/metrics`) liste **sessizce bayatlar** ve o yol SPA kabuğuna düşer. Kapanış: `Program.cs`'in `Map*` çağrılarından türetilen bir karşılaştırma ayağı.
