...
- 🟡 **`CakismaRozeti` dokunma hedefi cihazda 48,0 dp ÖLÇÜLDÜ** (162 px / density 540);
  ...
  karşılığı bulunamadı; yeni madde olarak eklendi, `HUKUM.md`'ye şerh düşüldü.)*
...
- 🟡 **İZLENMEYEN DOSYA: 80 [A8 commit'i sonrası; öncesi 78, 39'da 77, 38'de 92].** `KANIT/**/_tmp_*` · `KANIT/slice-3c/02-G2/` · kök `HUKUM.md` · **dört** ölü `src/client/test/_*`. 🔴 **`.gitignore` deseni ÖNERİLMEDİ:** `_` önekli bazı dosyalar doktrinin **kanıtıdır** (üstteki madde), geniş desen onları gizler. 🔴 **A8 İKİ YENİ ÖLÜ HUSK ÜRETTİ:** `_a8_olcum_test.dart` + `_a8_probe_test.dart` — ölçüm araçlarıydı, işleri bitti, `rm` bu ortamda **izinli değil** ⇒ **0 test'e boşaltıldılar** (ölçüldü: `test(` sayısı **0**, 428 sayımını etkilemiyor). Ölçümün kendisi `KANIT/A8/00-OLCUM.txt`'de **korundu**. Silme **Onur'da**.
