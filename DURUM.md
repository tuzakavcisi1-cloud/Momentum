...
🟢 **`W3` KISMEN İLERLEDİ (o71, `K186`):** `T3`'ün **CORP** ayağı kapandı — `D-W3-4` üründe koşuyor (`Web/IstemciServisi.cs`, `OnPrepareResponse`, **yalnız statikler**), 7 test + `M231`/`M231b` ısırıyor. `T1`/`T2` **zaten vardı** (eski *"implementasyon yok"* satırı ölü beyandı — `K185`). 🔴 Kalan: `T5` (`K158` erteledi) · `T6`/`T7` kapı araçları ve `T9` ADR (`K175` **PARK**) · `G43`/`G44` kapsamı ürünle **AYRIŞIK** (`B-O71-1`) · `G46` canlı HTTP **koşulmadı**. `B-O63-1`…`B-O63-5` **AÇIK**.
...
- 🔴 **`izolasyon-olc.py` `B`/`S`/`F` TASLAĞI DENETİMDE DÜŞTÜ (16 bulgu) ⇒ `araclar/`'a KONULMADI** (`B-O63-5`; `KANIT/W3/06-…o63.py`'de donmuş, onarım **AYRI ELE** — `K34-f`). Ders: kendi kapısını kendi altın kümesiyle kanıtlayan el, hâlâ kör bir kapı teslim edebilir. Ayrıntı: hafıza + `BORCLAR.md`.
- 🔒 **K160 — `BORCLAR.md` SIKIŞTIRILDI (Onur, o63):** dosya artık **indekstir** (kimlik + tek cümle); **39.086 → 17.669 b**, hiçbir borç **kapanmadı/silinmedi**. Gerekçe: hafıza K160.
...
| `iddia-kapisi.py` **1.3.0** [A9c] | saf çekirdek (`kanit_topla`/`denetle`); kanıt eşlemesi **yalnız HAM dosya adından** (`I2`/`I3`), envanter reddi (`LISTE_ESIGI=8`, iki yönlü pinli), `I1` satır-sha keyli muafiyet (`iddia-muafiyet.json`) | **27/27** |
...
| `belge-tavan-kapisi.py` (banner **1.0.0**, etiket bayat ⇒ `B-O50-2`) | canlı belge **bayt tavanı + PAY**; `T1` aşım (KIRMIZI) · `T2` dar pay (SARI, eşik %5) · `T0` dosya yok. Tavanı **kendi değiştirmez** (K40) — kapsam tablosundaki **her tavanı PİNLER** (vaka **10·13·14·15**). Gevşetmelerin ölçülmüş gerekçeleri **hafızada** (K117/K126/K151/K152/K160) | **15/15** |
| `oturum-sagligi.py` **1.0.0** | K21'in mekanik kapısı: kanonik eşik (`S1`) · yüzde avı (`S2`) · eşik kopyası (`S3`) · token+payda (`S4`/`S5`, `--transcript` ister, yoksa **OLCULMEDI**) · kimlik tazeliği (`D1`, `KIMLIKLER.md`+`DURUM.md` listesinden — K157). Çıkış 4 = kanonik temiz ama sağlık ölçülmedi. 🔴 **Mount'ta 45 sn tavanını AŞAR** [o70] ⇒ `D1` ayakları **tek tek** çağrılır, `S4`/`S5` **bulutta** koşar | **30/30** |
| `izolasyon-olc.py` **1.0.0** [`W3`] | çapraz-köken izolasyon: **H** ayağı (yalnız stdlib) `COOP`/`COEP` başlıklarını, **T** ayağı (playwright) **gerçek tarayıcıda** `crossOriginIsolated`'ı ölçer — bayrak yolu **ölü**. `credentialless` **bilerek** reddedilir; ulaşılamayan adres **ORTAM HATASI**'dır. 🔴 Onur'un makinesinde playwright **YOK** ⇒ `T` vakaları kapsam dışı, `N/M`'ye **girmez** (`B-O62-3`); buradaki sayı **cihazda** ölçülendir | **9/9** |
...
| `ci-kapisi.py` [`A13`] | CI iş akışının statik ayakları (`G28a`…`G30c`). 🔴 Envantere **o56'da eklendi**. 🟢 **o69 (`D-A13-4`):** `A13/G31/a`–`h` (backend CI işi) eklendi, `_CI_TEMIZ` fikstürü gerçek `ci.yml`'e hizalandı; `G31/h` (akış-stili YAML yasağı) bağımsız denetimdeki **kör kapıyı** kapattı (`KANIT/CI/09`) | **22/22** |
...
🔴 **ENVANTER o64'TE ÖLÇÜLDÜ (8 Ağu 2026): `araclar\` altında **31** dosya; **25** çalıştırılabilir (**24** `.py` + `verify.ps1`), tablo **25** satır; kalan **6** veri/yardımcı (3 `.json` · `.md` · `.sha256` · `.gitkeep`) + 2 dizin.** 🔴 `envantersiz-kapı` sınıfı **üç kez** ısırdı (`ci-kapisi` o53 · `ss2-kapisi` o55 · `cors-kapisi` o57); *envanterde olmayan kapı tetiklenemez* — sınıfın **hâlâ mekanik kapısı yok** (`B-O64-3`). Bayat-iddia anlatımı: **hafıza**.
