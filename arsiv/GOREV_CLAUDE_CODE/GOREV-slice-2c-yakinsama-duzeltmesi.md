...
8. **Geriye dönük veri: BACKFILL GEREKMİYOR — VARSAYIM DEĞİL, ÖLÇÜM.** *(19 Tem 2026, Cowork ölçtü.)* `docker-compose.yml`'in kalıcı volume'ü (`momentum_momentum-pgdata`, PG 17) gerçekten VAR ve **migration'lar oraya UYGULANMIŞ** — 8 tablonun tamamı mevcut (`__EFMigrationsHistory`, `outbox_messages`, `processed_operations`, `sync_client_clock`, `sync_gc_state`, `sync_orset_removes`, `sync_orset_tags`, `sync_scalar_meta`). **ANCAK tablolar BOŞ:** `sync_orset_tags = 0 satır`, `outbox_messages = 0 satır`, yinelenen `(entity,set,element,tag)` anahtarı = 0. Yani düşük damga taşıyan tek bir satır bile yok. *(Ölçüm yöntemi: throwaway `postgres:17-alpine` konteyneri volume'e bağlandı, salt-okur sorgu koşuldu, konteyner silindi; volume'e dokunulmadı.)*
   **Yine de:** build sırasında o volume'e veri yazılırsa (ör. API elle o DB'ye bağlanıp çalıştırılırsa) bu ölçüm geçersizleşir. Böyle bir şey yaptıysan **DUR ve Cowork'e sor** — kayıp damga kendiliğinden geri gelmez (GREATEST ancak o tag'e yeni add gelirse yükseltir).
