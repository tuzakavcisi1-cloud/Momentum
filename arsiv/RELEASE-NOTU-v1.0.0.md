...
aynı istemci, ama **backend yoktur**: veriler yalnız tarayıcıda kalır ve senkron rozetleri hiç
onaylanmaz. Senkron ve çakışma vitrini **yalnız** yukarıdaki docker paketinde görülür.

---

## 2. Android APK (varlık: `momentum-v1.0.0-emulator.apk`)

| | |
|---|---|
| Boyut | **59.953.214 bayt** |
| sha256 | `1c67f1f43f309a08535f1bbc33397610c5d5dce8abe3bc740535faf2c36da7d9` |
...
| Derlendiği commit | `ce630ec` (etiketten bir önceki; sonraki commit yalnız `DURUM.md`'yi değiştirir) |
...
Gerçek makinede (Windows 11 + Docker Desktop), 17 Ağustos 2026:
...
- CI kapıları yeşil: `ci #65` (`09d0e75` — etiketlenen commit) · `paket #8` (`84ff84c`) ·
  `pages #9` (`ce630ec`). `84ff84c` ile `09d0e75` arasındaki iki commit yalnız kanıt ve durum
  metnini değiştirir; ürün kodu aynıdır.

**NE ÖLÇÜLMEDİ:** gerçek arm64 cihazda paket kapısı koşulmadı (kırılma yalnız manifestle
gösterildi) · yatay (landscape) yerleşim telefonda denenmedi · erişilebilirlik duyuruları TalkBack
ile doğrulanmadı · iOS hiçbir cihazda koşmadı (yalnız CI'da derlenir).
...
`src/server/` — katmanlar (Api · Application · Domain · Infrastructure).
