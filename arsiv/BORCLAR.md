...
- 🟡 **`B-O71-5` — `SS2` KRİTER 8'İN ÇAKIŞMA EKRANI CANLI GÖRSELLE KANITLANMADI.** Uçtan uca koşum kabul edildi (`K187`) ama çakışma çözüm ekranının **cihazdaki görüntüsü** yakalanmadı; iki değerin aynı anda gösterildiği yalnız `G34/b` **widget testiyle** (mutantlı) ölçülü, A cihazının ekranı `[DOĞRULANMADI]`. Kanıt/beyan: `KANIT/SS2/13-KABUL-HUKMU-COWORK-kriter8-UCTAN-UCA.md` §3.
- 🟡 **`B-O71-6` — SSH.NET İÇİN GEÇİCİ AÇIK CVE PİNİ.** `GHSA-q939-rpr3-3284` (`SSH.NET <=2025.1.0`) 12 Ağu'da CVE kapısını yaktı; test projesine **geçici** açık pin (`2026.0.0`) kondu. Kapanış: Testcontainers bump'ında pin **kaldırılır** ve `verify.ps1` yeniden ölçülür.

### o71 PAGES DEMOSU BORÇLARI (canlı ölçüm `KANIT/o71/16-pages-demo/07-canli-olcum-COWORK.md`)
- 🔴 **`B-O71-7` — `sil` VERİ KATMANINDA VAR, ARAYÜZDE YOK.** `GorevDeposu.sil` (arayüz 81, uygulama 388) + `Gorevler.silindi` tombstone senkron protokolüne bağlı ve testli; `gorev_listesi_ekrani.dart` onu **hiç çağırmıyor** (çağrılanlar: `gorevlerGorunur`·`tamamlaGeriAl`·`duzenle`·`ekle`), `sunum/` altında `Dismissible` yok. Canlı demoda ziyaretçi **silemiyor** (14 Ağu 2026 ölçüldü). `K192` gereği kapatılmadı, `README`'de **beyan edildi**. Sınıf: *beyan ile ürün yüzeyinin ayrışması*. Kapanış: UI tetikleyicisi + widget testi + mutant.
- 🟡 **`B-O71-8` — `flutter test` DÖRT KANIT JSON'UNU HER KOŞUMDA YENİDEN YAZIYOR.** `KANIT/slice-3c/02-G2/{01-ekle,02-duzenle,03-tamamlaGeriAl,04-sil}.json` her koşumda **yeni UUID'lerle** üretiliyor ⇒ çalışma ağacı kirleniyor, kabul turunun kanıtı artık *o baytlar değil*. `B-O71-2`'nin **istemci tarafındaki kardeşi** (o backend'de aynı sınıf). Kapanış: kanıt `test/` çıktısına yazılsın, depoya değil.
- 🟡 **`B-O71-9` — `web-varlik.sha256` PİNİ DEMO YOLUNDA ÖLÇÜLMÜYOR.** `sqlite3.wasm` (748.424 b) + `drift_worker.js` (354.758 b) git'te izlenen hâliyle **kamuya iniyor**; `web-varlik-indir.py` demo zincirinde hiç koşmuyor. Kapanış: `pages.yml`'e yalnız-doğrulama ayağı ya da ayrı kapı.
- 🟡 **`B-O71-10` — CI'DAKİ `grep` AYAĞI WINDOWS'TA ÖLÇÜLEMİYOR.** `pages.yml`'in kapısı bash+grep; yerelde `findstr` körlüğü (o46) nedeniyle **semantik eşdeğer Python** koşuldu (`kapi-esdeger.py`). CI ayağının tek kanıtı ilk koşum logudur. Kapanış: kapının gövdesi tek bir `.py`'ye taşınıp CI'da da o çağrılsın.
- 🟡 **`B-O71-11` — DEMO YALNIZ TEK TARAYICI/TEK PROFİLDE ÖLÇÜLDÜ.** Anonim/gizli pencere, **Android Chrome** (`unsafeIndexedDb` beklentisi), Safari/iOS (7 günlük depolama tavanı), Firefox ve ilk boya süresi **ölçülmedi**; Cowork'ün bulut tarayıcısı `github.io`'ya çıkamadı (`ERR_CONNECTION_RESET`). `K161`: bir vaka ölçmek sınıf kapatmaz.
- 🟡 **`B-O71-12` — `DURUM.md` PAYI 662 BAYTA DÜŞTÜ.** o71 kapanışında `belge-tavan-kapisi` **SARI** (T2, dar pay). Budama ya da tavan kararı (`K40`: eşik **Onur'dan**) bir sonraki oturuma kaldı.
